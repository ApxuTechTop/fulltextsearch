...
TEST(SearcherTests, ExampleTest) {
	...
	searcher::Searcher indexSearcher(reader);
	std::vector<searcher::Result> results
		= indexSearcher.search("matrix revoadutions", config);
	/*
	std::stringstream sout;
	for (const auto [score, doc_id] : results) {
		sout << doc_id << " " << score << std::endl;
	}
	*/

	fs::remove_all(path_to_index);
	ASSERT_EQ(results[0].document_id, file_id3);
	ASSERT_EQ(results[1].document_id, file_id2);
}
