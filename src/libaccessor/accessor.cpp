...
std::string
accessor::TextIndexAccessor::load_document(const int document_id) const {
	const std::string doc_name{ std::to_string(document_id) };
	std::ifstream doc(path_to_index / docs_dir / doc_name);
	return std::string{ std::istreambuf_iterator<char>(doc),
						std::istreambuf_iterator<char>() };
}

std::size_t accessor::TextIndexAccessor::total_docs() const {
	return std::distance(fs::directory_iterator{ path_to_index / docs_dir },
...
accessor::TextIndexAccessor::get_term_infos(std::string term) const {
	...
	std::ifstream f(path_to_index / entries_dir
					/ (strstream.str().substr(0, hashsize)));
